...
plot(0, col='white', xlim=c(0,1), ylim=c(0,1), xlab = "control probability", ylab = "eczema probability")
c=1
for (i in c('m', 'w')) {
  for (j in c(50,18)) {
...
##
##  Clusters for the CONTROL group
##

pdf("dendrograms_eczema.pdf")
for (k in c(50,18)) {
  for (j in c('m', 'w')) {
    for (i in 0:1){
      ...
      tit <- paste0('Age ', k, ', ', ifelse(j=='m','men', 'women'), ': ', 
                    ifelse(i==0, 'matched controls', 'atopic eczema'), ' (complete linkage)')
...
for (k in c(50,18)) {
  for (j in c('m', 'w')) {
    for (i in 0:1){
      ...
      dhc0 <- color_branches(dhc0, h=0.5)
      tit <- paste0('Age ', k, ', ', ifelse(j=='m','men', 'women'), ': ', 
                    ifelse(i==0, 'matched controls', 'atopic eczema'), ' (Ward D2 linkage)')
...
# hc0e <- eclust(as.dist(t(md0)), "hclust", k = 4, hc_method="complete")
# fviz_nbclust(as.dist(t(md0)), FUN = hcut, method = "wss")
# fviz_dend(hc0e)
# fviz_silhouette(hc0e)
...
for (k in c(50,18)) {
  for (j in c('m', 'w')) {
    for (i in 0:1){
      ...
      tit <- paste0('Undirected network, 5-year co-occurrence probability age ', 
                    k, ', ', ifelse(j=='m','men', 'women'), ': ', 
                    ifelse(i==0, 'matched controls', 'atopic eczema'))
      net <- visNetwork(nodes, edges, main=tit) %>% 
        visNodes(shape="ellipse") %>% 
        visIgraphLayout(randomSeed = 1999) %>% 
        visEdges(widthConstraint =1, smooth =T, 
        scaling = list(min=1, max=20, customScalingFunction = 'function (min,max,total,value) {
                         if (max === min) {
                         return 0.5;
                         }
                         else {
                         var scale = 1 / (1 - 0.3);
                         return Math.max(0,(value*1.2 - 0.3)*scale);
                         }
                         }'))
      print(net)
    }
