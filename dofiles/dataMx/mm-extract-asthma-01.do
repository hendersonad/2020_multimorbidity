...
DESCRIPTION OF FILE:	Aim is to identify asthma exposed patients
						asthma exposed defined using a combination of Read 
						...
						with a primary care morbidity code for asthma.
						
MORE INFORMATION:	
	Runs as follows:
		1. Uses CPRD Fast tool (define) to identify patients with a Read code for 
			asthma.
		2. Combines these patients with individuals with a ICD-10 record for asthma
			in HES (identified as having an asthma code ever in HES by CPRD).
		3. Extract data for all those with either a primary or secondary care 
			morbidity code for asthma.
	I note	from one of Harriet's files that all the individuals she IDd 
	from HES records also had a CPRD record for asthma.
	However, I also note that she only included as potential cases those with
	an asthma code recorded in the primary diagnostic position.
	This is not appropriate for this study - need to ID anyone who has ever had
	a morbidity code for asthma recorded at any time in either HES or CPRD.
					
DATASETS USED:		medcodes-asthmaDx		// medcode list for asthma - NB: used 
											// code list from HF, but checked to make sure OK
											// by searching in Read code browser
DATASETS CREATED: 	CPRD extract files 	// ecxa1 (all those with an asthma code)
										// exca2_linked (asthma ICD/Read code and eligible for HES linkage)
					patients	// dataset of patids and dates for patients
								// who are eligible for linkage who have a 
								// CPRD/HES record for asthma
					
					All_patids_allcodes // insheet of patids and first Dx dates of individuals 
										// with asthma Dx in HES ever from CPRD
...
/*******************************************************************************
#1. Identify list of patients with Read code for asthma
	using CPRD Fast define tool
*******************************************************************************/
/* run in first version of this file exca-extract01v2-asthma no need to run again */
...
rungprddlg, define(1) extract(0) build(July 2017) ///
	...
	inc1filename(${pathCodelists}/medcodes-asthmaDx) inc1codevar(medcode) /// file and variable containing code list (asthmaDx has approx 10 codes)
...
#2.  Add in patients identified from HES 
	(provided by CPRD)
*******************************************************************************/
* insheet text file from CPRD including patids and date of first asthma Dx
* in HES of individuals with a ICD-10 asthma code ever
insheet using ${pathIn}/All_patids_allcodes.txt, clear // Provided by CPRD?? 44,701 rows of data
prog_formatdate, datevar(epistart)

label data "patids of pxs with asthma ICD-10 code ever"
notes: patids of pxs with asthma ICD-10 code ever
notes: $filename / TS
compress

save ${pathIn}/All_patids_allcodes, replace

* merge with individuals identified with an asthma code recorded in CPRD
...
HESbeforeCP |
         RD |      Freq.     Percent        Cum.
------------+-----------------------------------
          1 |      3,794       14.04       14.04 // n=3,794 with both HES and CPRD code for asthma where HES code before CPRD code
          . |     23,238       85.96      100.00
------------+-----------------------------------
      Total |     27,032      100.00
...
#3. Restrict to those eligible for HES/ONS linkage
	...
	2020 edit 1 - don't do this bit for multimorbidity and asthma cluster cohorts. Don't need HES linkage 
*******************************************************************************/
...
* save
sort patid
label data "linked patients with CPRD/HES asthma record"
notes: linked patients with CPRD/HES asthma record
notes: ${filename} / TS
compress
